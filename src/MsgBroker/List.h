...
public:
    ...
    /** \fn insertItem
     *  \brief Insert an item to the list at the current location (_search pointer)
     *  \param item Item to add
     *  \param exception Exception error code
     */
    void insertItem(T* item, List::Exception *exception = 0);
     ...
     *  \return Pointer to the last item or NULL if empty
     */
    T* getLastItem();
    ...
    /** \fn getPrevItem
     *  \brief Get prev item in list
     *  \return Pointer to the prev item or NULL if no more
     */
    T* getPrevItem();

    /** \fn getCurrentItem
     *  \brief Get current item in list
     *  \return Pointer to the current item or NULL if no more
     */
    T* getCurrentItem();
