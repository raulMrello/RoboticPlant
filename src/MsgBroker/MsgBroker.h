...
public:
    ...
    struct Observer{
		...
		void (FPointerDummy::*method_callback)(const char * topicname, void * topicdata);
    };
    ...
    static MsgBroker::Topic * installTopic(const char* name, uint32_t datasize, MsgBroker::Exception *exception = 0){
        ...
        topic = (MsgBroker::Topic*)Heap::MALLOC(sizeof(MsgBroker::Topic));
        if(!topic){
            THROW(OUT_OF_MEMORY);
            MUTEX_UNLOCK();
            return 0;
    ...
    static MsgBroker::Observer * attach (const char * topic_name, void *subscriber, NotifyCallback notify, MsgBroker::Exception *exception = 0){
        ...
        obs->notify = notify;
		obs->method_callback = (void (FPointerDummy::*)(const char *, void *))0;
     ...
     */
	template<class T>
    static MsgBroker::Observer * attach (const char * topic_name, T *subscriber, void (T::*method)(const char *, void *), MsgBroker::Exception *exception = 0){
        THROW(NO_ERRORS);
        if(!MsgBroker::topic_list){
            THROW(DEINIT);
            return 0;
    ...
    static void publish (const char * topic_name, void *data, uint32_t datasize, MsgBroker::Exception *exception = 0){
        ...
        while(obs){
            ...
            if(obs->notify){
				...
				(((FPointerDummy*)obs->observer)->*obs->method_callback)(topic->name, topic->data);
			}
            obs = topic->observer_list->getNextItem();
        }
        MUTEX_UNLOCK();
